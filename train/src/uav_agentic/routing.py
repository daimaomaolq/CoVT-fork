...
def preliminary_suspicions(
    ...
    if area < config.small_area_threshold:
...
class DependencyAwareRouter:
    def plan(
        ...
        if config.method in {Method.ONE_PASS, Method.CONFIDENCE_GATED, Method.PARENT_ONLY}:
            return plan

        needs_target = bool({
            "parse_risk", "low_token_confidence", "shape_risk",
            "needs_target_verification", "candidate_ambiguity",
        }.intersection(suspicions))
        needs_context = graph.has_context and bool({
            "needs_context_verification", "needs_relation_verification",
            "candidate_ambiguity",
        }.intersection(suspicions))
        ...
        if needs_target and "target" not in config.disabled_agents:
            reasons = [
                reason for reason in suspicions
                if reason in {
                    "parse_risk", "low_token_confidence", "shape_risk",
                    "needs_target_verification", "candidate_ambiguity",
                }
        ...
        if needs_zoom and "zoom" not in config.disabled_agents:
            ...
            action_candidates.append((
                "zoom",
                _utility(1.0, 0.60 if "small_object_risk" in reasons else 0.40, 1.0),
                reasons,
            ))
        ...
        plan.run_relation_reasoner = (
            "relation" not in config.disabled_agents
            and (
                graph.has_relation
